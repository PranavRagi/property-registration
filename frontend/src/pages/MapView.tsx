...
const s: Record<string, React.CSSProperties> = {
  page:           { background: 'transparent', minHeight: 'calc(100vh - 120px)' },
  errorCard:      { maxWidth: 440, margin: '80px auto', background: 'var(--color-surface)', borderRadius: 'var(--radius-lg)', padding: 'var(--space-8) var(--space-6)', boxShadow: 'var(--shadow-md)', textAlign: 'center', border: '1px solid var(--color-border)' },
  code:           { background: 'var(--color-surface-muted)', padding: '2px 6px', borderRadius: 'var(--radius-sm)', fontFamily: 'monospace', fontSize: 12 },
  codeBlock:      { background: 'var(--color-primary-dark)', color: '#4ec9b0', padding: 'var(--space-3) var(--space-4)', borderRadius: 'var(--radius-sm)', fontFamily: 'monospace', fontSize: 13, textAlign: 'left' },
  tokenLink:      { display: 'inline-block', marginTop: 'var(--space-4)', color: 'var(--color-accent)', fontSize: 13, textDecoration: 'none', fontWeight: 500 },
  header:         { display: 'flex', alignItems: 'center', padding: 'var(--space-3) var(--space-5)', background: 'var(--color-surface)', borderBottom: '1px solid var(--color-border)', flexWrap: 'wrap', gap: 'var(--space-3)', borderRadius: 'var(--radius-md) var(--radius-md) 0 0', boxShadow: 'var(--shadow-sm)' },
  title:          { fontSize: 17, fontWeight: 700, color: 'var(--color-text)', marginRight: 4, whiteSpace: 'nowrap', letterSpacing: '-0.01em' },
  searchWrap:     { display: 'flex', alignItems: 'center', flex: 1, minWidth: 220, maxWidth: 380, background: 'var(--color-surface-muted)', border: '1px solid var(--color-border)', borderRadius: 20, padding: '0 var(--space-3)', gap: 6 },
  searchInput:    { flex: 1, border: 'none', background: 'transparent', outline: 'none', fontSize: 13, padding: '8px 0', color: 'var(--color-text)' },
  clearBtn:       { background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-text-muted)', fontSize: 14, padding: 0, flexShrink: 0 },
  searchInfo:     { padding: '6px var(--space-5)', background: 'var(--color-warning-bg)', borderBottom: '1px solid #f0e6c0', fontSize: 12, color: 'var(--color-text-muted)' },
  filters:        { display: 'flex', gap: 6, flexWrap: 'wrap' },
  filterBtn:      { padding: '6px var(--space-3)', background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 20, fontSize: 12, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4, color: 'var(--color-text-secondary)', whiteSpace: 'nowrap', fontWeight: 500 },
  filterActive:   { background: 'var(--color-primary)', color: 'white', border: '1px solid var(--color-primary)', fontWeight: 600 },
  filterCount:    { background: 'rgba(255,255,255,0.2)', borderRadius: 10, padding: '1px 5px', fontSize: 10, fontWeight: 700 },
  map:            { width: '100%', height: 'calc(100vh - 220px)', minHeight: 400 },
  ...
  loadingBox:     { textAlign: 'center', padding: 28, background: 'var(--color-surface)', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-md)', border: '1px solid var(--color-border)' },
  overlay:        { position: 'fixed', inset: 0, background: 'rgba(26,37,47,0.45)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 },
  popup:          { background: 'var(--color-surface)', borderRadius: 'var(--radius-lg)', width: 310, position: 'relative', boxShadow: 'var(--shadow-xl)', overflow: 'hidden', maxHeight: '90vh', overflowY: 'auto', border: '1px solid var(--color-border)' },
  popupClose:     { position: 'absolute', top: 8, right: 8, background: 'rgba(0,0,0,0.45)', border: 'none', color: 'white', width: 28, height: 28, borderRadius: '50%', cursor: 'pointer', fontSize: 13, zIndex: 1 },
  stat:           { background: 'var(--color-surface-muted)', borderRadius: 'var(--radius-sm)', padding: 'var(--space-2) 10px' },
  statL:          { fontSize: 10, color: 'var(--color-text-muted)', marginBottom: 2, fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.04em' },
  statV:          { fontSize: 13, fontWeight: 600, color: 'var(--color-text)' },
  popupCtaUnlock: { width: '100%', padding: '10px 0', background: 'var(--color-primary)', color: 'white', border: 'none', borderRadius: 'var(--radius-sm)', fontSize: 13, fontWeight: 600, cursor: 'pointer' },
  popupCtaTour:   { width: '100%', padding: '10px 0', background: 'linear-gradient(135deg, var(--color-success) 0%, #1e8449 100%)', color: 'white', border: 'none', borderRadius: 'var(--radius-sm)', fontSize: 13, fontWeight: 600, cursor: 'pointer' },
  legend:         { display: 'flex', gap: 'var(--space-4)', padding: 'var(--space-2) var(--space-5)', background: 'var(--color-surface)', borderTop: '1px solid var(--color-border)', flexWrap: 'wrap', alignItems: 'center', borderRadius: '0 0 var(--radius-md) var(--radius-md)' },
}